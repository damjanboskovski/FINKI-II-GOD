package Laboratoriski.labs1.zad3;
...
final class IntegerArray{
    ...
    public IntegerArray(int[] arr) { this.arr = Arrays.copyOf(arr, arr.length); }

    public int length(){  return this.arr.length; }
    public int getElementAt(int i){ return this.arr[i]; }
    public int sum() {
        int sumElements = 0;
    ...
    public double average() { return (double) sum()/length(); }
    public IntegerArray getSorted(){
        return new IntegerArray(Arrays.copyOf(Arrays.stream(this.arr).sorted().toArray(),length()));
    }
    public IntegerArray concat(IntegerArray ia){
        int[] newArr = new int[ia.length() + this.length()];
        for(int i=0; i<length(); i++){ newArr[i] = this.arr[i]; }
        for(int i=length(); i<length()+ia.length(); i++){ newArr[i] = ia.arr[i - length()]; }
